async function page(env) {
  const r = await env.DB.prepare("SELECT chunk FROM static_blobs WHERE name='admin-v9-final' ORDER BY seq").all();
  const b64 = (r.results || []).map(x => x.chunk).join('');
  if (!b64) return new Response('Admin page not found', { status: 500 });
...
export default {
  async fetch(req, env) {
    ...
    if (u.pathname === '/' || u.pathname === '/index.html') return page(env);
