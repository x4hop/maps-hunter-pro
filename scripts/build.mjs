...
const inline=async(html,base='')=>{html=html.replace(/<link rel="stylesheet" href="([^"]+)">/g,(_,src)=>`@@STYLE:${src}@@`);html=html.replace(/<script src="([^"]+)" defer><\/script>/g,(_,src)=>`@@SCRIPT:${src}@@`);for(const m of [...html.matchAll(/@@STYLE:([^@]+)@@/g)])html=html.replace(m[0],`<style>${await read(base+m[1].replace(/^\//,''))}</style>`);for(const m of [...html.matchAll(/@@SCRIPT:([^@]+)@@/g)])html=html.replace(m[0],`<script>${(await read(base+m[1].replace(/^\//,''))).replace(/<\/script/gi,'<\\/script')}</script>`);return html};
