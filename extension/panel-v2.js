...
function renderState(next){
  ...
  document.body.classList.toggle('is-running',running);if($('phaseBadge'))$('phaseBadge').textContent=phase;if($('statusTitle'))$('statusTitle').textContent=paused?'Search paused':scanning?'Scanning Google Maps':extracting?'Extracting Maps + website contacts':readyToExtract?'Ready to extract':total?'Process complete':'Ready to scan';if($('statusText'))$('statusText').textContent=state.status||'Add tags and start scanning.';if($('progressBar'))$('progressBar').style.width=extracting&&queued?`${Math.min(100,Math.round(processed/queued*100))}%`:scanning?'18%':readyToExtract?'100%':'0%';
