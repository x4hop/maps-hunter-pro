...
for name in ['0001_initial.sql','0002_auth_activation_codes.sql','0003_atomic_entitlements.sql','0004_operations_privacy.sql']:
    db.executescript((root/'backend'/'migrations'/name).read_text())
required={'users','auth_sessions','admin_sessions','activation_requests','activation_codes','entitlement_grants','usage_events','data_requests','payout_commissions'}
